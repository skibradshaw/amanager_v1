  	 ...
  	 <h3>{{ $payment->payment_type }} Payment {{ $payment->paid_date->format('n/j/Y') }}: ${{ number_format($payment->amount,2) }} by {{ $payment->method }} {{ (!empty($payment->check_no)) ? " #" . $payment->check_no : "(# missing)" }}</h3>	
